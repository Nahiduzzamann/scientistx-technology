  ...
  return (
    <group rotation={[-Math.PI / 2, 0, Math.PI]} {...props} dispose={null}>
      ...
      <mesh geometry={nodes['URF-Height_Lampd_0'].geometry} material={materials.Lampd} material-color="lightgreen">
        <Marker rotation={[0, Math.PI / 2, 0]} position={[0, 1.2, 0]}>
          {/* Anything in here is regular HTML, these markers are from font-awesome */}
          {/* <FaMapMarkerAlt style={{ color: 'orange' }} /> */}
        </Marker>
        <group position={[0, 0, 1.2]} rotation={[0, 0, Math.PI]}>
          <Marker rotation={[0, Math.PI / 2, Math.PI / 2]}>
            {/* <div style={{ position: 'absolute', fontSize: 10, letterSpacing: -0.5, left: 17.5 }}>ScientistX</div> */}
...
export default function Viewer() {
  ...
  return (
    <Canvas camera={{ position: [5, 0, 0], fov: 50 }}>
      ...
      {loaded&&(<Model position={[0, 0.25, 0]} />)}
      {loaded&&(<Environment preset="city" />)}
      {loaded&&(<ContactShadows frames={1} scale={5} position={[0, -1, 0]} far={1} blur={5} opacity={0.5} color="#204080" />)}
      <OrbitControls />
    </Canvas>
  )
}
